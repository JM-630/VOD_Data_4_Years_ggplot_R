...
for (x in 1:Jan) {
  ...
  site <- read.csv("/Users/student/Documents/VOD_Project/Swiss_Site1452.csv", header=TRUE)
...
if (y == 1) {
  graph <- graph + geom_line(data = VOD_Extract2, aes(y = VOD, color = "2017"))
  Year <- "2018"
}
if (y == 2) {
  graph <- graph + geom_line(data = VOD_Extract2, aes(y = VOD, color = "2018"))
  Year <- "2019"
}
if (y == 3) {
  graph <- graph + geom_line(data = VOD_Extract2, aes(y = VOD, color = "2019"))
}
